...
object crm_hive_es {
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setMaster("yarn").setAppName("crm_hive_es")
    ...
    var value: RDD[(String, collection.Map[String, AnyRef])] = EsSpark.esRDD(spark.sparkContext, "tag", query)
    var esRdd: RDD[String] = value.map(x => {
      (x._2.getOrElse("account_id", ""), x._2.getOrElse("city", ""), x._2.getOrElse("age", 0))
    }).filter(x => {
      !x._1.equals("")
    }).map(x => x._1.toString)

    var account_info_cache = esRdd.toDF("account_id").cache()
    val account_info=account_info_cache.createTempView("account_info")
